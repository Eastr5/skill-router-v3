---
name: "figura"
description: "Create publication-quality figures and diagrams for academic papers with a render-view-fix loop. Invoke when user wants publication-ready plots, architecture diagrams, or to fix/beautify a figure."
---
...
- **LaTeX engine**: MiKTeX 25.12 — `pdflatex`, `xelatex`, `latexmk` all on PATH. Compile TikZ via:
  ```bash
  pdflatex -interaction=nonstopmode my_fig.tex
  ```
- **Chinese support**: `ctex` package pre-installed — use `\usepackage{ctex}` freely inside `.tex` sources
- **Pre-installed LaTeX packages relevant to this skill**: `tikz`, `pgfplots`, `tikz-bayesnet`, `tikz-cd`, `smartdiagram`, `forest`, `algorithm2e`, `helvet`
- **Rasterizer**: `pdftoppm` (poppler) for PDF→PNG previews; `pdfinfo` for page count; `pdftotext` for quick content check. Standard one-shot preview:
  ```bash
  pdftoppm -r 300 -png my_fig.pdf my_fig    # writes my_fig-1.png
  ```
- **Visual audit**: The `Read` tool can display PNGs. **Always view the compiled PNG** with Read before declaring a figure done — this is the iteration step in the render→view→fix loop.
- **Companion skills** (in this same `.trae/skills/` folder):
  - `tikz-diagrams-guide` — ready-to-adapt TikZ patterns (Transformer, MLP, 3D, Bayesian, commutative)
  - `academic-research-skills/visualization_agent` — for matplotlib/seaborn/ggplot2 data plots (bar, violin, heatmap, forest plot)
- **MCPs available**:
  - `mcp_drawio-official_open_drawio_mermaid` / `open_drawio_csv` — for flowchart / system diagrams via Mermaid/CSV
  - `mcp_latex-mcp-server_*` — **see warning below**
  - `mcp_MySQL_*` / `mcp_paper-search_*` / `mcp_semantic-scholar_*` — for data sourcing (not figure rendering)
  - `mcp_GitHub_*` — for publishing to a repo (CI-friendly figure generation)

> **⚠️ latex-mcp-server MCP is a placeholder on this machine.** The MCP is configured to run `python -m http.server 8080` and the `compile_latex` / `read_pdf` / `list_tex_files` tools will return errors. **Do not call these tools — use `RunCommand` with `pdflatex` / `pdftoppm` / `pdftotext` instead.** Other latex-mcp-server tools (`extract_bibliography`, `download_bibliography`) may work but have schema issues — fall back to Glob/Read/Write.
